...
import { backupAllToJson } from '../helpers/backup';
import { registerBackupHooks } from '../helpers/backupPlugin';
...
export async function connectMongoDb(dbName: string, uri: string = defaultUri): Promise<void> {
  try {
    if (mongoose.connection.readyState === 0) {
      ...
      // Daftarkan backup hook secara dinamis pada semua model yang telah ter-import
      registerBackupHooks();

      // Jalankan backup awal sesaat setelah berhasil terhubung
      backupAllToJson().catch((err) => console.error('Initial backup failed:', err));
    }
  } catch (error) {
